...
import destinations from "../../data/destinations"; // Ensure this file has the correct data
...
const DestinationsPage = () => {
  ...
  const [destinations, setDestinations] = useState([]); // Initial empty state for destinations
  ...
  // Fetch destinations from the backend (replace this with your actual API endpoint)
  useEffect(() => {
    axios
      .get("https://localhost:8000/api/destinations") // Replace with your API URL
      .then((response) => {
        setDestinations(response.data); // Assuming the response is an array of destinations
      })
      .catch((error) => {
  ...
  }, [searchQuery, activeFilters, activeSort, destinations]); // Add 'destinations' here to trigger the effect when data changes
            ...
            <div className="search-results-info">
              <p>
                ...
                {activeFilters.length > 0 &&
                  ` with ${activeFilters.length} filters applied`}
              </p>
            </div>
          )}
